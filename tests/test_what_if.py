...
def tcost_model(volumes, sigma):
    return TcostModel(half_spread=0.0005, nonlin_coeff=1.0, volume=volumes, sigma=sigma,)


@pytest.fixture()
def hcost_model():
    return HcostModel(borrow_costs=0., dividends=0.)
...
def test_attribution(returns, volumes, sigma, tcost_model, hcost_model):
    ...
    tcost = TcostModel(half_spread=0.0005, nonlin_coeff=1.0, volume=volumes, sigma=sigma)
    hcost = HcostModel(borrow_costs=0., dividends=0.)
    market_sim = simulator.MarketSimulator(
        returns=returns, costs=[tcost, 
        hcost
    ], volumes=volumes
    )
    ...
    assert np.allclose(attr["RMS error"], np.zeros(len(noisy.v)), atol=1e-7)
...
def test_attribute_non_profit_scalar(returns, sigma, volumes):
    ...
    tcost_model_sim = TcostModel(half_spread=0.0005, nonlin_coeff=1.0, volume=volumes, sigma=sigma,)
    hcost_model_sim = HcostModel(0.0)
    ...
    market_sim = simulator.MarketSimulator(
        returns=returns, costs=[tcost_model_sim, hcost_model_sim])
