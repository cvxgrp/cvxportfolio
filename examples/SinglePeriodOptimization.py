...
simulated_tcost = cp.TcostModel(half_spread=0.0005 / 2.0, nonlin_coeff=1.0, sigma=sigmas, volume=volumes)
simulated_hcost = cp.HcostModel(borrow_costs=0.0001)
simulator = cp.MarketSimulator(returns=returns, costs=[simulated_tcost, simulated_hcost], volumes=volumes, cash_key="USDOLLAR")
...
optimization_tcost = cp.TcostModel(half_spread=0.0005 / 2.0, nonlin_coeff=1.0, sigma=sigma_estimate, volume=volume_estimate)
optimization_hcost = cp.HcostModel(borrow_costs=0.0001)

idyos = pd.read_csv(datadir + "idyo_vars.csv.gz", index_col=0, parse_dates=[0])
exposures = pd.read_csv(datadir + "exposures.csv.gz", index_col=[0,1], parse_dates=[0])
factor_sigma = pd.read_csv(datadir + "factor_sigma.csv.gz", index_col=[0,1], parse_dates=[0])

# risk_data = pd.HDFStore(datadir + "risk_model.h5")
risk_model = cp.FactorModelRisk(exposures=exposures, factor_Sigma=factor_sigma, idyosync=idyos, use_last_available_time=True)
...
for gamma_risk in gamma_risks_coarse:
    for gamma_tcost in gamma_tcosts_coarse:
        policies[(gamma_risk, gamma_tcost)] = cp.SinglePeriodOpt(
            return_estimate, [gamma_risk * risk_model, gamma_tcost * optimization_tcost, optimization_hcost],
            [cp.LeverageLimit(3)], solver='ECOS')

warnings.filterwarnings("ignore")
results.update(dict(zip(policies.keys(), simulator.legacy_run_multiple_backtest(1e8 * w_b,
                start_time=start_t, end_time=end_t, policies=policies.values(),
                parallel=True))))
...
results.update(dict(zip(policies.keys(),
            simulator.legacy_run_multiple_backtest(1e8 * w_b,
                start_time=start_t, end_time=end_t, policies=policies.values(), parallel=True))))
