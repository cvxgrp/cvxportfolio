...
class AlphaBetaReturnForecaster():

    def __init__(self, market='^GSPC'):
        self.market_returns = cvx.YahooFinance(market).data['return']

    def values_in_time(self, past_returns, t, **kwargs):
        past_market = self.market_returns.loc[self.market_returns.index < t]
        mean_market = past_market.mean()
        Xty = past_returns.iloc[:, :-1].multiply(past_market, axis=0).mean()
        print(Xty)
        mults = []
        for name in past_returns.columns[:-1]:
            its_market = past_market.loc[past_returns[name].dropna().index]
            X = pd.DataFrame({'market': its_market})
            X['intercept'] = 1
            mult = np.linalg.solve((X.T @ X)/len(X), [mean_market, 1.])
            # print(mult)
            mults.append(mult)
        mults = np.array(mults).T
        print(mults)
        guy = Xty.values * mults[0] + past_returns.iloc[:, :-1].mean().values * mults[1]
        print('new', guy)
        print('old', past_returns.iloc[:, :-1].mean().values)
        return guy

forecaster = AlphaBetaReturnForecaster()
...
def policy(gamma_risk, gamma_trade):
    ...
    return cvx.SinglePeriodOptimization(
        cvx.ReturnsForecast(forecaster
        )
