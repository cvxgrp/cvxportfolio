...
class MaxTrade(BaseTradeConstraint):
    ...
    def __init__(self, volumes, max_fraction_of_volumes=0.05):
        ...
        self.max_fraction_of_volumes = ParameterEstimator(max_fraction_of_volumes)
        self.children = [self.volumes, self.max_fraction_of_volumes]


    def compile_to_cvxpy(self, w_plus, z, portfolio_value):
        """Return a list of Cvxpy constraints."""
        return [
            cvx.multiply(cvx.abs(z[:-1]), portfolio_value) <= cvx.multiply(self.volumes.parameter, self.max_fraction_of_volumes.parameter)
        ]
