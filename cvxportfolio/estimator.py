    ...
    def _values_in_time(self, **kwargs):
        ...
        for _, subestimator in self.__dict__.items():
            if hasattr(subestimator, "_values_in_time"):
                subestimator._values_in_time(**kwargs)
...
class DataEstimator(PolicyEstimator):
    ...
    by its `_values_in_time` method, which is the way `cvxportfolio`
    ...
    Args:
        data (object, pandas.Series, pandas.DataFrame): Data expressed
            ...
            pass a callable object which implements the _values_in_time method
        ...
        use_last_available_time (bool): if the pandas index exists
            and is a pandas.DateTimeIndex you can instruct self._values_in_time
    ...
    def _pre_evaluation(self, universe, backtest_times):
        ...
        if self.compile_parameter:
            value = self.internal__values_in_time(t=backtest_times[0])
    ...
    def value_checker(self, result):
        ...
        Args:
            result (int, float, or np.array): data produced by self._values_in_time
        ...
        if np.isscalar(result):
            if np.isnan(result) and not self.allow_nans:
                raise MissingValuesError(
                    f"{self.__class__.__name__}._values_in_time result is a np.nan scalar."
                )
            else:
                return result
        ...
        if isinstance(result, np.ndarray):
            if np.any(np.isnan(result)) and not self.allow_nans:
                message = f"{self.__class__.__name__}._values_in_time result is an array with np.nan's."
        ...
        raise DataError(
            f"{self.__class__.__name__}._values_in_time result is not a scalar or array."
        )

    def internal__values_in_time(self, t, *args, **kwargs):
        """Internal method called by `self._values_in_time`."""

        if hasattr(self.data, "_values_in_time"):
            return self.value_checker(
                self.data._values_in_time(
                    t, *args, **kwargs))
        ...
        if (
            ...
            try:
                ...
                raise MissingValuesError(
                    f"{self.__class__.__name__}._values_in_time could not find data for requested time."
                )
    ...
    def _values_in_time(self, t, *args, **kwargs):
        ...
        """
        self.current_value = self.internal__values_in_time(t, *args, **kwargs)
...
#         self.value = self.internal__values_in_time(t, *args, **kwargs)
#
#
...
#
#     def _pre_evaluation(self, universe, backtest_times):
#         value = super().internal__values_in_time(t=backtest_times[0])
#         self.parameter = cp.Parameter(
#             value if hasattr(value, "shape") else (),
#             PSD=self.positive_semi_definite, nonneg=self.non_negative)
#
#     def _values_in_time(self, t, **kwargs):
#         self.parameter.value = self.internal__values_in_time(t, **kwargs)
...
#         super()._pre_evaluation(universe, backtest_times)
#         value = super()._values_in_time(t=backtest_times[0])
#         super().__init__(value.shape if hasattr(value, "shape") else (),
#             PSD=self.positive_semi_definite, nonneg=self.non_negative)
#
#     def _values_in_time(self, t, **kwargs):
#         """Update Cvxpy Parameter value."""
#         self.value = super()._values_in_time(t=t, **kwargs)
