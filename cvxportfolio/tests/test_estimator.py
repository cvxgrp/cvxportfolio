...
class TestEstimators(unittest.TestCase):
    ...
    def test_callable(self):
        ...
        self.assertEqual(estimator._values_in_time(time), 1.0)

        estimator = DataEstimator(PlaceholderCallable(np.nan))
        with self.assertRaises(MissingValuesError):
            estimator._values_in_time(t=time)

        data = np.arange(10.0)
        estimator = DataEstimator(PlaceholderCallable(data))
        self.assertTrue(np.all(estimator._values_in_time(t=time) == data))

        data[1] = np.nan
        with self.assertRaises(MissingValuesError):
            estimator._values_in_time(time)
    ...
    def test_scalar(self):
        ...
        self.assertTrue(estimator._values_in_time(t=time) == 1.0)
    ...
    def test_series_dataframe_notime(self):
        ...
        estimator = DataEstimator(data)
        self.assertTrue(np.all(estimator._values_in_time(t=time) == data.values))
    ...
    def test_series_timeindex(self):
        ...
        print(estimator._values_in_time("2022-01-05"))
        self.assertTrue(estimator._values_in_time("2022-01-05") == data.loc["2022-01-05"])

        with self.assertRaises(MissingValuesError):
            estimator._values_in_time("2022-02-05")

        estimator = DataEstimator(data, use_last_available_time=True)
        self.assertTrue(estimator._values_in_time("2022-02-05") == data.iloc[-1])

        with self.assertRaises(MissingValuesError):
            estimator._values_in_time("2021-02-05")

        data["2022-01-05"] = np.nan
        estimator = DataEstimator(data)
        self.assertTrue(estimator._values_in_time("2022-01-04") == data.loc["2022-01-04"])
        with self.assertRaises(MissingValuesError):
            estimator._values_in_time("2022-01-05")
    ...
    def test_dataframe_timeindex(self):
        ...
        print(estimator._values_in_time("2022-01-05"))
        self.assertTrue(np.all(estimator._values_in_time(
            "2022-01-05") == data.loc["2022-01-05"]))

        with self.assertRaises(MissingValuesError):
            estimator._values_in_time("2021-01-05")

        estimator = DataEstimator(data, use_last_available_time=True)
        self.assertTrue(np.all(estimator._values_in_time("2022-02-05") == data.iloc[-1]))
    ...
    def test_dataframe_multindex(self):
        ...
        self.assertTrue( np.all(estimator._values_in_time(
            "2022-01-05") == data.loc["2022-01-05"]))
        with self.assertRaises(MissingValuesError):
            estimator._values_in_time("2020-01-05")

        index = pd.MultiIndex.from_product([second_level, timeindex])
        data = pd.DataFrame(np.random.randn(len(index), 10), index=index)
        estimator = DataEstimator(data)
        assert np.all(estimator._values_in_time("2020-01-05") == data.values)
